...
public class HashMap<Key, Value> : IEnumerable<Pair<Key, Value>>
{
    ...
    public bool PutIfAbsent(Key key, Value value)
    {
        ...
        Pair<Key, Value> pair = new Pair<Key, Value>();
        pair.Key = key;
        pair.Value = value;
        _values[hashCode].Add(pair);
        _keys.Add(key);
        return true;
    }
    //Ćwiczenie 3
...
public class Product
{
    ...
    public override int GetHashCode()
    {
        ...
        int price = Price.GetHashCode();
        int quantity = Quantity.GetHashCode();
        int unit = Unit.GetHashCode();
